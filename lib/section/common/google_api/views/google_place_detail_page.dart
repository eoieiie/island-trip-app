...
class GooglePlaceDetailPage extends StatelessWidget {
  ...
  const GooglePlaceDetailPage({super.key, required this.place}); // 생성자에서 place 인스턴스 받음
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 10), // 섹션과 다음 요소 사이에 간격 추가

              // 장소 이름
              Text('장소 이름: ${place.name}', style: const TextStyle(fontSize: 16)),
              const SizedBox(height: 10), // 이름과 다음 요소 사이에 간격 추가

              // 주소
              Text('주소: ${place.address}', style: const TextStyle(fontSize: 16)),
              const SizedBox(height: 10),
              // 평점
              if (place.rating != null)
                Text('평점: ${place.rating}', style: const TextStyle(fontSize: 16)),
              const SizedBox(height: 10),
              ...
              if (place.phoneNumber != null)
                Text('전화번호: ${place.phoneNumber}', style: const TextStyle(fontSize: 16)),
              const SizedBox(height: 10),

              // 웹사이트
              if (place.website != null)
                Text('웹사이트: ${place.website}', style: const TextStyle(fontSize: 16)),
              const SizedBox(height: 10),

              // 위도
              if (place.latitude != null)
                Text('위도: ${place.latitude}', style: const TextStyle(fontSize: 16)),
              const SizedBox(height: 10),

              // 경도
              if (place.longitude != null)
                Text('경도: ${place.longitude}', style: const TextStyle(fontSize: 16)),
              const SizedBox(height: 10),
              ...
              if (place.photoUrls != null && place.photoUrls!.isNotEmpty)
                Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 10), // 사진과 다음 요소 사이에 간격 추가
                    ListView.builder(
                      shrinkWrap: true, // 리스트뷰가 부모 위젯 크기를 벗어나지 않도록 설정
                      physics: const NeverScrollableScrollPhysics(), // 리스트뷰 자체 스크롤 방지
                ...
                const Text('사진이 없습니다.', style: TextStyle(fontSize: 16)), // 사진이 없을 경우 출력
            ],
          ),
        ),
      ),
