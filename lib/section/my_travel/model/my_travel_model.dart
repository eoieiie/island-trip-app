...
class TravelModel {
  ...
  TravelModel({
    ...
    DateTime? updatedAt,
  }) : updatedAt = updatedAt ?? DateTime.now(); // 필수 파라미터로 마지막 업데이트 날짜를 받습니다.
  ...
  // 여행 상태를 계산하는 getter 추가
  String get travelStatus {
    final today = DateTime.now();
    if (today.isBefore(startDate)) {
      return '여행 출발까지 ${startDate.difference(today).inDays}일!';
    } else if (today.isAfter(endDate)) {
      return '다녀온 지 ${today.difference(endDate).inDays}일';
    } else {
      return '여행 ${today.difference(startDate).inDays + 1}일차';
    }
  }
}
